...
- **`hydra tree`** — the full ASCII render. Use it only when the user asks for
  the complete tree. Ordinary question turns show the current branch from
  `resume.hydrated`; see **Asking**.
...
1. **The current branch** — render only `resume.hydrated` (the root-to-`next`
   ancestry) in a fenced block. Every turn, no exceptions. Do not run or print
   `hydra tree` unless the user asks for the complete tree.
